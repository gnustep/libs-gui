/** <title>GSNifLoader</title>

   Loader for Native Interface Format (NIF) files.
...
@interface NSObject (GSNifAwaking)
- (void) awakeFromNib;
@end

/*
 * NIF deliberately uses only property-list types.  A dictionary containing
 ...
 */
@interface GSNifDecoder : NSObject
{
      ...
      if (className != nil)
        {
          ...
          if (identifier == nil || [identifier length] == 0)
            [NSException raise: NSInvalidArgumentException
                        format: @"NIF object %@ has no $id", value];
          if ([_objects objectForKey: identifier] != nil)
            [NSException raise: NSInvalidArgumentException
                        format: @"Duplicate NIF object id '%@'", identifier];
          objectClass = NSClassFromString(className);
          if (objectClass == Nil)
            [NSException raise: NSInvalidArgumentException
                        format: @"Unknown NIF class '%@'", className];

          object = [[objectClass allocWithZone: _zone] init];
          if (object == nil)
            [NSException raise: NSInvalidArgumentException
                        format: @"Could not initialize NIF object '%@' as %@",
...
- (id) objectForReference: (NSString *)identifier
{
  ...
  if (object == nil)
    [NSException raise: NSInvalidArgumentException
                format: @"Unknown NIF object reference '%@'", identifier];
  return object;
}
      ...
      if (type != nil)
        {
          ...
          [NSException raise: NSInvalidArgumentException
                      format: @"Unknown NIF value type '%@'", type];
        }
  ...
  while ((connection = [enumerator nextObject]) != nil)
    {
      ...
      if ([kind isEqualToString: @"outlet"])
        ...
        [NSException raise: NSInvalidArgumentException
                    format: @"Unknown NIF connection kind '%@'", kind];
...
@interface GSNifLoader : GSModelLoader
@end

@implementation GSNifLoader
...
+ (BOOL) canReadData: (NSData *)data
{
  ...
  return ([header rangeOfString: @"<key>format</key>"].location != NSNotFound
          && [header rangeOfString: @"<string>NIF</string>"].location != NSNotFound);
}
  ...
  NS_DURING
    {
      ...
      if (![document isKindOfClass: [NSDictionary class]]
          || ![[document objectForKey: @"format"] isEqualToString: @"NIF"]
          || [[document objectForKey: @"version"] integerValue] != 1)
        [NSException raise: NSInvalidArgumentException
                    format: @"Invalid or unsupported NIF document"];
      if (![[document objectForKey: @"objects"] isKindOfClass: [NSArray class]]
        ...
        [NSException raise: NSInvalidArgumentException
                    format: @"NIF objects and topLevelObjects must be arrays"];

      loaded = [[[[GSNifDecoder alloc] initWithDocument: document
                                                context: context
                                                   zone: zone] autorelease] decode];
    }
  NS_HANDLER
    {
      NSLog(@"Exception occurred while loading NIF: %@", [localException reason]);
    }
  NS_ENDHANDLER
