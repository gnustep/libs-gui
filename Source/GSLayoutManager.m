...
static void run_cache_attributes(glyph_run_t *r,NSDictionary *attributes)
{
	...
	r->font=[attributes objectForKey: NSFontAttributeName];
	if (!r->font)
...
static void run_free_attributes(glyph_run_t *r)
{
	[r->font release];
}

static void run_copy_attributes(glyph_run_t *dst,const glyph_run_t *src)
{
...
-(void) _initGlyphs
{
	int i;
	glyph_run_head_t *h;
	glyphs=malloc(sizeof(glyph_run_head_t)*(SKIP_LIST_DEPTH-1)+sizeof(glyph_run_t));
	memset(glyphs,0,sizeof(glyph_run_head_t)*(SKIP_LIST_DEPTH-1)+sizeof(glyph_run_t));
...
#if BENCHMARK
static NSDictionary *dummy_dict;
static SEL attrAtIdxSel;
static IMP attrAtIdx;
if (!dummy_dict)
{
	dummy_dict=[[NSDictionary alloc] initWithObjectsAndKeys:
		[NSFont userFontOfSize: 0],NSFontAttributeName,
		nil];
/*	attrAtIdxSel=@selector(attributesAtIndex:effectiveRange:);
	attrAtIdx=[_textStorage methodForSelector: attrAtIdxSel];*/
}
#endif
...
/*		{
		...
		run_cache_attributes(new,attributes);
